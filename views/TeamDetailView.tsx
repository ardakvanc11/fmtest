...
const TeamDetailView = ({ team, allTeams, fixtures, currentDate, manager, onClose, onPlayerClick, onTeamClick }: TeamDetailViewProps) => {
    ...
    // --- GET TRANSFERS FROM REAL DATA & CALCULATE TOTALS ---
    const transfers = useMemo(() => {
        const history = team.transferHistory || [];
        
        const arrivals = history.filter(t => t.type === 'BOUGHT').map(t => ({
            date: t.date,
            name: t.playerName,
            from: t.counterparty,
            price: t.price
        ...
        const departures = history.filter(t => t.type === 'SOLD').map(t => ({
            date: t.date,
            name: t.playerName,
            to: t.counterparty,
            price: t.price
        ...
        // Calculate Totals
        const totalSpent = arrivals.reduce((sum, item) => {
            const val = parseFloat(item.price.replace(' M€', '')) || 0;
            return sum + val;
        }, 0);

        const totalIncome = departures.reduce((sum, item) => {
            const val = parseFloat(item.price.replace(' M€', '')) || 0;
            return sum + val;
        }, 0);
        ...
        return { arrivals, departures, totalSpent, totalIncome, netBalance };
    }, [team.transferHistory]);
    
    // Updated history logic: Use hardcoded history if available, otherwise mock
    const history = useMemo(() => {
        if (team.leagueHistory && team.leagueHistory.length > 0) {
            // Sort history descending by year
            return [...team.leagueHistory].reverse();
        }
    ...
    return (
        <div className="h-full bg-slate-100 dark:bg-slate-900 overflow-y-auto custom-scrollbar flex flex-col">
            ...
            {/* --- TOP MENU NAVIGATION (UPDATED TO HOMEVIEW STYLE) --- */}
            <div className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700/50 sticky top-0 z-30 shrink-0">
                <div className="max-w-7xl mx-auto px-2 flex items-center">
                    {/* Geri Butonu - Her zaman sol başta */}
                    <button
                        onClick={onClose}
                        className="flex items-center gap-1 px-4 py-3 text-slate-500 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 transition-colors border-r border-slate-200 dark:border-slate-700 mr-2 shrink-0 group"
                    >
                    ...
                    <div className="flex items-center gap-1 overflow-x-auto no-scrollbar pt-2 flex-1">
                        {tabs.map((t) => {
                            ...
                            return (
                                ...
                                >
                                    {isActive && (
                                        <div className="absolute top-0 left-0 right-0 h-[3px] bg-yellow-500 dark:bg-yellow-400 rounded-t-full shadow-[0_1px_8_rgba(250,204,21,0.5)]"></div>
                                    )}
                ...
                <div className="p-4 md:p-8 max-w-7xl mx-auto">
                    <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10">
                        <div className="relative group">
                            ...
                            {team.logo ? (
                                <img src={team.logo} className="w-24 h-24 md:w-32 md:h-32 object-contain drop-shadow-2xl relative z-10" alt={team.name} />
                            ) : (
                                <div className={`w-24 h-24 md:w-32 md:h-32 rounded-full ${team.colors[0]} flex items-center justify-center text-4xl font-bold text-white relative z-10 border-4 border-white dark:border-slate-800`}>
                                    {team.name.charAt(0)}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex-1 text-center md:text-left">
                            ...
                            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-slate-600 dark:text-slate-300">
                                <div className="flex gap-1.5 items-center font-bold text-yellow-600 dark:text-yellow-500">
                                    ...
                                    <span className="text-lg">{team.championships} Şampiyonluk</span>
                                </div>
                                ...
                                <div className="flex gap-0.5">
                                    {[...Array(5)].map((_, i) => (
                                        <Star key={i} size={18} className={`${i < Math.floor(reputation) ? 'fill-yellow-500 text-yellow-500' : 'text-slate-300 dark:text-slate-700'}`} />
                                    ))}
                                    <span className="ml-2 text-sm font-bold text-yellow-600 dark:text-yellow-500">{reputation.toFixed(1)}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="hidden lg:flex flex-col items-center justify-center p-6 bg-slate-900 dark:bg-black rounded-2xl border-b-4 border-yellow-500 shadow-xl min-w-[140px]">
                            <div className="text-[10px] text-slate-500 font-bold uppercase mb-1">Takım Gücü</div>
                            <div className="text-5xl font-black text-white font-teko leading-none">{Math.round(team.strength)}</div>
                            <div className="w-full h-1 bg-slate-800 mt-3 rounded-full overflow-hidden">
                                <div className="bg-yellow-500 h-full" style={{width: `${team.strength}%`}}></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {activeTab === 'GENERAL' && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                            ...
                            <StatCard label="Form Durumu" value={
                                <div className="flex gap-1 items-center justify-start h-full">
                                    {form.length > 0 ? form.map((r, i) => (
                                        <div key={i} className={`w-3 h-3 rounded-full ${r === 'W' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]' : r === 'D' ? 'bg-slate-400' : 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]'}`} title={r}></div>
                                    )) : <span className="text-sm font-normal text-slate-400">Veri yok</span>}
                                </div>
                            } subValue="Son 5 Maç" icon={Activity} />
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2 uppercase tracking-widest">
                                <Users size={20} className="text-indigo-500" /> Öne Çıkan Oyuncular
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                <PlayerStatCard 
                                    label="Gol Kralı" player={topScorer} statValue={topScorer.seasonStats.goals} statLabel="Gol" icon={Goal} colorClass="text-green-500" onClick={onPlayerClick}
                                />
                                <PlayerStatCard 
                                    label="Asist Kralı" player={topAssister} statValue={topAssister.seasonStats.assists} statLabel="Asist" icon={Zap} colorClass="text-blue-500" onClick={onPlayerClick}
                                />
                                <PlayerStatCard 
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                            ...
                            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                                    <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                        <Calendar className="text-blue-500" size={20}/> Gelecek Maçlar
                                    </h3>
                                </div>
                                <div className="divide-y divide-slate-100 dark:divide-slate-700">
                                    {futureMatches.slice(0, 8).map(f => {
                                        const opponentId = f.homeTeamId === team.id ? f.awayTeamId : f.homeTeamId;
                                        const opponent = allTeams.find(t => t.id === opponentId);
                                        const isHome = f.homeTeamId === team.id;
                                        return (
                                            <div key={f.id} onClick={() => onTeamClick(opponentId)} className="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition cursor-pointer">
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white ${isHome ? 'bg-green-600' : 'bg-red-600'}`}>
                                                        {isHome ? 'EV' : 'DEP'}
                                                    </div>
                                                    <div>
                                                        <div className="font-bold text-slate-900 dark:text-white">{opponent?.name}</div>
                                                        <div className="text-xs text-slate-500">{getFormattedDate(f.date).label} • {getRelativeTime(f.date)}</div>
                                                    </div>
                                                </div>
                                                <div className="text-sm font-mono text-slate-400">20:00</div>
                                            </div>
                                        );
                                    })}
                                </div>
                        ...
                        {pastMatches.length > 0 && (
                            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                                    <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                        <History className="text-slate-500" size={20}/> Sonuçlar
                                    </h3>
                                </div>
                                <div className="divide-y divide-slate-100 dark:divide-slate-700">
                                    {pastMatches.map(f => {
                                        ...
                                        let resColor = "bg-slate-500";
                                        if (myScore > oppScore) resColor = "bg-green-600";
                                        else if (myScore < oppScore) resColor = "bg-red-600";
                                        else resColor = "bg-yellow-500";

                                        return (
                                            <div key={f.id} onClick={() => onTeamClick(opponentId)} className="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition cursor-pointer">
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-8 h-8 rounded flex items-center justify-center text-sm font-bold text-white ${resColor}`}>
                                                        {isHome ? `${f.homeScore}-${f.awayScore}` : `${f.awayScore}-${f.homeScore}`}
                                                    </div>
                                                    <div>
                                                        <div className="font-bold text-slate-900 dark:text-white">{opponent?.name}</div>
                                                        <div className="text-xs text-slate-500">{getFormattedDate(f.date).label} • {isHome ? 'İç Saha' : 'Deplasman'}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        );
                        ...
                        {teamFixtures.length === 0 && <div className="text-center text-slate-500 py-10">Fikstür bulunamadı.</div>}
                    </div>
                )}
                ...
                {activeTab === 'TRANSFERS' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="bg-red-50 dark:bg-red-900/10 p-4 rounded-xl border border-red-200 dark:border-red-800/30 flex items-center justify-between">
                                <div>
                                    <p className="text-xs font-bold text-red-600 dark:text-red-400 uppercase">Toplam Harcama</p>
                                    <p className="text-2xl font-black text-red-700 dark:text-red-300 font-mono">-{transfers.totalSpent.toFixed(1)} M€</p>
                                </div>
                                <div className="p-2 bg-red-200 dark:bg-red-800 rounded-lg text-red-700 dark:text-red-200">
                                    <ArrowRight size={24}/>
                                </div>
                            </div>

                            <div className="bg-emerald-50 dark:bg-emerald-900/10 p-4 rounded-xl border border-emerald-200 dark:border-emerald-800/30 flex items-center justify-between">
                                <div>
                                    <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase">Toplam Gelir</p>
                                    <p className="text-2xl font-black text-emerald-700 dark:text-emerald-300 font-mono">+{transfers.totalIncome.toFixed(1)} M€</p>
                                </div>
                                <div className="p-2 bg-emerald-200 dark:bg-emerald-800 rounded-lg text-emerald-700 dark:text-emerald-200">
                                    <ArrowLeft size={24}/>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                                <div className="p-4 bg-emerald-50 dark:bg-emerald-900/10 border-b border-emerald-100 dark:border-emerald-800/30 flex items-center justify-between">
                                    <h3 className="font-bold text-emerald-800 dark:text-emerald-400 flex items-center gap-2">
                                        <ArrowRight className="bg-emerald-200 dark:bg-emerald-800 rounded-full p-1 w-6 h-6"/> Gelenler
                                    </h3>
                                    <span className="text-xs font-bold text-emerald-600 dark:text-emerald-500 uppercase">Bu Sezon</span>
                                </div>
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm text-left">
                                        <thead className="bg-slate-50 dark:bg-slate-900 text-xs text-slate-500 uppercase font-bold">
                                            <tr>
                                                <th className="px-4 py-3">Tarih</th>
                                                <th className="px-4 py-3">Oyuncu</th>
                                                <th className="px-4 py-3">Geldiği Takım</th>
                                                <th className="px-4 py-3 text-right">Bedel</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                            {transfers.arrivals.length === 0 ? (
                                                <tr><td colSpan={4} className="p-4 text-center text-slate-500 italic">Henüz gelen oyuncu yok.</td></tr>
                                            ) : transfers.arrivals.map((t, i) => (
                                                <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition">
                                                    ...
                                                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300 flex items-center gap-2">
                                                        <ArrowLeft size={12} className="text-green-500"/> {t.from}
                                                    </td>
                                                    <td className="px-4 py-3 text-right font-mono font-bold text-emerald-600 dark:text-emerald-400">{t.price}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                            ...
                            {/* Departures */}
                            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                                <div className="p-4 bg-red-50 dark:bg-red-900/10 border-b border-red-100 dark:border-red-800/30 flex items-center justify-between">
                                    <h3 className="font-bold text-red-800 dark:text-red-400 flex items-center gap-2">
                                        <ArrowRight className="bg-red-200 dark:bg-red-800 rounded-full p-1 w-6 h-6"/> Gidenler
                                    </h3>
                                    <span className="text-xs font-bold text-red-600 dark:text-red-500 uppercase">Bu Sezon</span>
                                </div>
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm text-left">
                                        <thead className="bg-slate-50 dark:bg-slate-900 text-xs text-slate-500 uppercase font-bold">
                                            <tr>
                                                <th className="px-4 py-3">Tarih</th>
                                                <th className="px-4 py-3">Oyuncu</th>
                                                <th className="px-4 py-3">Gittiği Takım</th>
                                                <th className="px-4 py-3 text-right">Bedel</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                            {transfers.departures.length === 0 ? (
                                                <tr><td colSpan={4} className="p-4 text-center text-slate-500 italic">Henüz giden oyuncu yok.</td></tr>
                                            ) : transfers.departures.map((t, i) => (
                                                <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition">
                                                    <td className="px-4 py-3 text-slate-500 whitespace-nowrap text-xs">{t.date}</td>
                                                    <td className="px-4 py-3 font-bold text-slate-900 dark:text-white">{t.name}</td>
                                                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300 flex items-center gap-2">
                                                        <ArrowRight size={12} className="text-red-500"/> {t.to}
                                                    </td>
                                                    <td className="px-4 py-3 text-right font-mono font-bold text-emerald-600 dark:text-emerald-400">{t.price}</td>
                                                </tr>
                                            ))}
                ...
                {activeTab === 'HISTORY' && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-8 text-center border border-slate-700 shadow-xl relative overflow-hidden">
                            ...
                            <h3 className="text-xl font-bold text-yellow-500 uppercase tracking-widest mb-8 relative z-10 flex justify-center items-center gap-3">
                                <Trophy size={24}/> Kulüp Müzesi
                            </h3>
                            
                            <div className="flex justify-center gap-12 flex-wrap relative z-10">
                                <div className="flex flex-col items-center gap-2">
                                    <div className="w-24 h-24 bg-yellow-500/10 rounded-full flex items-center justify-center border-2 border-yellow-500 shadow-[0_0_20px_rgba(234,179,8,0.3)]">
                                    ...
                                    <div className="text-xs uppercase text-slate-400 font-bold tracking-wider">Lig Şampiyonluğu</div>
                                </div>
                                
                                <div className="flex flex-col items-center gap-2">
                                    <div className="w-24 h-24 bg-blue-500/10 rounded-full flex items-center justify-center border-2 border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.3)]">
                                        <Trophy size={48} className="text-blue-400 fill-blue-400"/>
                                    </div>
                                    <div className="text-4xl font-black text-white">{team.domesticCups || 0}</div>
                                    <div className="text-xs uppercase text-slate-400 font-bold tracking-wider">Türkiye Kupası</div>
                                </div>

                                <div className="flex flex-col items-center gap-2">
                                    <div className="w-24 h-24 bg-slate-500/10 rounded-full flex items-center justify-center border-2 border-slate-500 shadow-[0_0_20px_rgba(148,163,184,0.3)]">
                                        <Trophy size={48} className="text-slate-300 fill-slate-300"/>
                                    </div>
                                    <div className="text-4xl font-black text-white">{team.superCups || 0}</div>
                                    <div className="text-xs uppercase text-slate-400 font-bold tracking-wider">Süper Kupa</div>
                                </div>

                                {/* Avrupa Kupası */}
                                <div className="flex flex-col items-center gap-2">
                                    <div className="w-24 h-24 bg-purple-500/10 rounded-full flex items-center justify-center border-2 border-purple-500 shadow-[0_0_20px_rgba(168,85,247,0.3)]">
                                        <Trophy size={48} className="text-purple-400 fill-purple-400"/>
                                    </div>
                                    <div className="text-4xl font-black text-white">{team.europeanCups || 0}</div>
                                    <div className="text-xs uppercase text-slate-400 font-bold tracking-wider">Avrupa Kupası</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                                <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                    <History className="text-slate-500" size={20}/> Lig Geçmişi
                                </h3>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm text-left">
                                    <thead className="bg-slate-100 dark:bg-slate-900 text-xs text-slate-500 uppercase font-bold">
                                        <tr>
                                            <th className="px-6 py-3">Sezon</th>
                                            <th className="px-6 py-3">Lig</th>
                                            <th className="px-6 py-3 text-center">Sıralama</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                        {history.map((h, i) => (
                                            <tr key={i} className={`hover:bg-slate-50 dark:hover:bg-slate-700/30 transition ${h.rank === 1 ? 'bg-yellow-50 dark:bg-yellow-900/10' : ''}`}>
                                                <td className="px-6 py-3 font-mono font-bold text-slate-700 dark:text-slate-300">{h.year}</td>
                                                <td className="px-6 py-3 text-slate-600 dark:text-slate-400">Süper Toto Ligi</td>
                                                <td className="px-6 py-3 text-center">
                                                    {h.rank === 1 ? (
                                                        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 font-bold text-xs border border-yellow-200 dark:border-yellow-700">
                                                            <Trophy size={10} className="fill-yellow-600"/> ŞAMPİYON
                                                        </span>
                                                    ) : (
                                                        <span className={`font-bold ${h.rank <= 4 ? 'text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-slate-400'}`}>
                                                            {h.rank}.
                                                        </span>
                                                    )}
                                                </td>
