...
const PlayerDetailView: React.FC<PlayerDetailViewProps> = ({ player, onClose, myTeamId, manager, teammates, onInteract, onUpdatePlayer, onStartNegotiation, onStartTransferNegotiation, onReleasePlayer, currentWeek }) => {
    ...
    const handleToggleTransferList = () => {
        if (onUpdatePlayer) {
            // Optimistik UI update için yerel state beklemeyelim, parent update edecek
            onUpdatePlayer(player.id, { transferListed: !player.transferListed });
            
            // Kullanıcıya bildirim
            if (!player.transferListed) {
                // Listeye ekleniyor
                // alert(`${player.name} transfer listesine eklendi.`); // Opsiyonel, UI zaten değişecek
            } else {
                // Listeden çıkıyor
            }
            // Dropdown'ı kapat
    ...
    const handleRestConfirm = () => {
        ...
        if (cond > 80) {
            ...
            if (onUpdatePlayer) onUpdatePlayer(player.id, { condition: Math.min(100, cond + 30) });
        }
        setInteractionResult({ text: response, mood });
    };
    ...
    const getAttrColor = (val: number) => { if (val >= 90) return 'text-yellow-500 font-black'; if (val >= 80) return 'text-green-500 font-bold'; if (val >= 70) return 'text-blue-500 font-bold'; if (val >= 40) return 'text-slate-600 dark:text-slate-300 font-medium'; return 'text-red-500'; };
    ...
    const AttributeRow = ({ label, value }: { label: string, value: number }) => (
        <div className="flex justify-between items-center py-1.5 border-b border-slate-100 dark:border-slate-700/30 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-700/20 px-1 rounded transition-colors"><span className="text-[13px] text-slate-500 dark:text-slate-400 font-medium uppercase tracking-tight">{label}</span><span className={`text-base font-mono ${getAttrColor(value)}`}>{value}</span></div>
    );
        ...
        if (activeDropdown === 'TRANSFER') {
            ...
            return (
                <div className={sectionClass}>
                    ...
                    {/* ADDED: Allow Transfer List Toggle from Dropdown for quick access */}
                    {isMyPlayer && (
    ...
    return (
        <div className="h-full bg-slate-100 dark:bg-slate-900 overflow-y-auto custom-scrollbar flex flex-col" onClick={() => activeDropdown && setActiveDropdown(null)}>
            ...
            <div className="flex-1 p-4 md:p-8 max-w-7xl mx-auto w-full space-y-6 pb-20">
                ...
                {activeTab === 'GENERAL' && (<div className="space-y-6 animate-in fade-in slide-in-from-bottom-2"><div className="grid grid-cols-1 lg:grid-cols-3 gap-6"><div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm"><h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2 border-b border-slate-100 dark:border-slate-700 pb-2"><Swords className="text-blue-500" size={20}/> TEKNİK</h3><div className="space-y-0.5"><AttributeRow label="Bitiricilik" value={player.stats.finishing} /><AttributeRow label="Dripling" value={player.stats.dribbling} /><AttributeRow label="İlk Kontrol" value={player.stats.firstTouch} /><AttributeRow label="Kafa Vuruşu" value={player.stats.heading} /><AttributeRow label="Korner" value={player.stats.corners} /><AttributeRow label="Markaj" value={player.stats.marking} /><AttributeRow label="Orta Yapma" value={player.stats.crossing} /><AttributeRow label="Pas" value={player.stats.passing} /><AttributeRow label="Penaltı Kullanma" value={player.stats.penalty} /><AttributeRow label="Serbest Vuruş" value={player.stats.freeKick} /><AttributeRow label="Teknik" value={player.stats.technique} /><AttributeRow label="Top Kapma" value={player.stats.tackling} /><AttributeRow label="Uzaktan Şut" value={player.stats.longShots} /><AttributeRow label="Uzun Taç" value={player.stats.longThrows} /></div></div><div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm"><h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2 border-b border-slate-100 dark:border-slate-700 pb-2"><Zap className="text-yellow-500" size={20}/> ZİHİNSEL</h3><div className="space-y-0.5"><AttributeRow label="Agresiflik" value={player.stats.aggression} /><AttributeRow label="Cesaret" value={player.stats.bravery} /><AttributeRow label="Çalışkanlık" value={player.stats.workRate} /><AttributeRow label="Karar Alma" value={player.stats.decisions} /><AttributeRow label="Kararlılık" value={player.stats.determination} /><AttributeRow label="Konsantrasyon" value={player.stats.concentration} /><AttributeRow label="Liderlik" value={player.stats.leadership} /><AttributeRow label="Önsezi" value={player.stats.anticipation} /><AttributeRow label="Özel Yetenek" value={player.stats.flair} /><AttributeRow label="Pozisyon Alma" value={player.stats.positioning} /><AttributeRow label="Soğukkanlılık" value={player.stats.composure} /><AttributeRow label="Takım Oyunu" value={player.stats.teamwork} /><AttributeRow label="Topsuz Alan" value={player.stats.offTheBall} /><AttributeRow label="Vizyon" value={player.stats.vision} /></div></div><div className="space-y-6"><div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm"><h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2 border-b border-slate-100 dark:border-slate-700 pb-2"><Activity className="text-red-500" size={20}/> FİZİKSEL</h3><div className="space-y-0.5"><AttributeRow label="Çeviklik" value={player.stats.agility} /><AttributeRow label="Dayanıklılık" value={player.stats.stamina} /><AttributeRow label="Denge" value={player.stats.balance} /><AttributeRow label="Güç" value={player.stats.physical} /><AttributeRow label="Hız" value={player.stats.pace} /><AttributeRow label="Hızlanma" value={player.stats.acceleration} /><AttributeRow label="Vücut Zindeliği" value={player.stats.naturalFitness} /><AttributeRow label="Zıplama" value={player.stats.jumping} /></div></div><div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm"><h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2 border-b border-slate-100 dark:border-slate-700 pb-2"><Heart className="text-purple-500" size={20}/> DURUM</h3><div className="space-y-4"><div><div className="flex justify-between mb-1 text-xs font-bold uppercase text-slate-500"><span>Kondisyon</span><span className={currentCondition < 60 ? 'text-red-500' : 'text-green-500'}>{Math.round(currentCondition)}%</span></div><div className="h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden"><div className={`h-full ${currentCondition < 50 ? 'bg-red-500' : currentCondition < 80 ? 'bg-yellow-500' : 'bg-green-500'}`} style={{width: `${currentCondition}%`}}></div></div></div><div className="grid grid-cols-2 gap-3"><div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-lg text-center"><div className="text-2xl font-black text-yellow-600">{player.seasonStats.averageRating || '-'}</div><div className="text-[10px] font-bold text-slate-500 uppercase">Reyting</div></div><div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-lg text-center"><div className="text-2xl font-black text-indigo-600">{player.morale}</div><div className="text-[10px] font-bold text-slate-500 uppercase">Moral</div></div></div>{player.injury && (<div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-3 rounded-lg flex items-start gap-3"><AlertTriangle className="text-red-500 shrink-0 mt-0.5" size={18}/><div><div className="text-xs font-bold text-red-700 dark:text-red-400 uppercase">Sakatlık Raporu</div><div className="text-sm font-bold text-red-600 dark:text-red-300 mt-0.5">{player.injury.type}</div><div className="text-[10px] text-slate-500 mt-1">{player.injury.daysRemaining} gün kaldı.</div></div></div>)}</div></div></div></div><div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch"><div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 md:p-8 shadow-sm flex flex-col items-center justify-center"><h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6 w-full border-b border-slate-100 dark:border-slate-700 pb-2 flex items-center gap-2 uppercase tracking-tight"><Trophy className="text-yellow-500" size={24}/> POZİSYONLAR</h3><PlayerPositionPitch player={player} /></div><div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 md:p-8 shadow-sm flex flex-col"><h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6 w-full border-b border-slate-100 dark:border-slate-700 pb-2 flex items-center gap-2 uppercase tracking-tight"><TrendingUp className="text-green-500" size={24}/> SEZON İSTATİSTİKLERİ</h3><PlayerStatsTable player={player} /></div></div></div>)}
                ...
                {activeTab === 'DEVELOPMENT' && (<div className="space-y-6 animate-in fade-in slide-in-from-right-2"><div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-8 shadow-sm text-center"><h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center justify-center gap-2"><TrendingUp className="text-green-500"/> Gelişim Raporu</h3><div className="flex justify-center gap-8 mb-8"><div className="text-center"><div className="text-xs text-slate-500 uppercase font-bold mb-1">Mevcut Yetenek</div><div className="text-4xl font-black text-blue-600 dark:text-blue-400">{player.skill}</div></div><div className="w-px bg-slate-200 dark:bg-slate-700"></div><div className="text-center"><div className="text-xs text-slate-500 uppercase font-bold mb-1">Potansiyel</div><div className="text-4xl font-black text-slate-400 dark:text-slate-500">{player.potential}</div></div></div><div className="w-full bg-slate-100 dark:bg-slate-900 h-4 rounded-full overflow-hidden relative mb-2"><div className="h-full bg-slate-300 dark:bg-slate-700 absolute left-0 top-0" style={{width: `${player.potential}%`}}></div><div className="h-full bg-blue-500 absolute left-0 top-0" style={{width: `${player.skill}%`}}></div></div><div className="flex justify-between text-xs text-slate-500 font-bold uppercase"><span>Mevcut ({player.skill})</span><span>Potansiyel ({player.potential})</span></div><div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4"><div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700"><div className="text-sm font-bold text-slate-900 dark:text-white mb-1">Antrenman Puanı</div><div className="text-2xl font-bold text-green-500">8.4 / 10</div></div><div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700"><div className="text-sm font-bold text-slate-900 dark:text-white mb-1">Odak Alanı</div><div className="text-lg font-bold text-blue-500">Dayanıklılık</div></div><div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700"><div className="text-sm font-bold text-slate-900 dark:text-white mb-1">Antrenör Görüşü</div><div className="text-xs text-slate-500 italic">{player.age > 30 ? "Fiziksel olarak düşüşte ancak tecrübesiyle açığı kapatıyor." : player.potential > player.skill + 5 ? "Hala gelişime çok açık, potansiyeline ulaşması için zamana ihtiyacı var." : "Potansiyeline ulaşmış durumda, formunu korumaya odaklanmalı."}</div></div></div></div></div>)}

                {activeTab === 'COMPARE' && (<div className="space-y-6 animate-in fade-in slide-in-from-right-2"><div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-8 shadow-sm flex flex-col items-center justify-center min-h-[400px]"><Scale size={64} className="text-slate-300 dark:text-slate-600 mb-4"/><h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Oyuncu Kıyaslama</h3><p className="text-slate-500 dark:text-slate-400 text-center max-w-md">Bu özellik şu anda geliştirme aşamasındadır. Yakında ligdeki diğer oyuncularla detaylı veri analizi yapabileceksiniz.</p><div className="mt-8 w-full max-w-lg"><div className="flex justify-between items-center mb-2"><span className="text-sm font-bold text-slate-700 dark:text-slate-300">{player.name}</span><span className="text-sm font-bold text-slate-500">Lig Ortalaması</span></div><div className="space-y-3">{[{ label: 'Hız', val: player.stats.pace, avg: 65 },{ label: 'Teknik', val: player.stats.technique, avg: 60 },{ label: 'Pas', val: player.stats.passing, avg: 58 },{ label: 'Güç', val: player.stats.physical, avg: 62 },].map((stat, i) => (<div key={i}><div className="flex justify-between text-xs mb-1"><span>{stat.label}</span></div><div className="h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden relative"><div className="absolute top-0 bottom-0 bg-slate-400 w-1 z-10" style={{left: `${stat.avg}%`}}></div><div className="h-full bg-blue-500" style={{width: `${stat.val}%`}}></div></div></div>))}</div></div></div></div>)}
