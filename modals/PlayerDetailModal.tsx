            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in duration-200">
                ...
                <div className="p-6 grid grid-cols-2 gap-8 text-slate-900 dark:text-white">
                    <div className="space-y-3">
                        <h4 className="text-red-600 dark:text-red-500 font-bold uppercase text-sm border-b border-slate-200 dark:border-slate-700 pb-1 mb-3 flex items-center gap-2"><Zap size={14}/> Fiziksel</h4>
                        ...
                        <div className="flex justify-between text-sm"><span>Çeviklik</span><span className="font-bold">{player.stats.agility}</span></div>
                        <div className="flex justify-between text-sm text-green-600 dark:text-green-400">
                            <span className="flex items-center gap-1"><Heart size={14}/> Kondisyon</span>
                            <span className="font-bold">{Math.round(currentCondition)}%</span>
                        </div>
                    </div>
                    <div className="space-y-3">
                        <h4 className="text-blue-600 dark:text-blue-500 font-bold uppercase text-sm border-b border-slate-200 dark:border-slate-700 pb-1 mb-3 flex items-center gap-2"><Swords size={14}/> Teknik & Zihinsel</h4>
                        <div className="flex justify-between text-sm"><span>Bitiricilik</span><span className="font-bold">{player.stats.finishing}</span></div>
                        <div className="flex justify-between text-sm"><span>Pas</span><span className="font-bold">{player.stats.passing}</span></div>
                        <div className="flex justify-between text-sm"><span>Vizyon</span><span className="font-bold">{player.stats.vision}</span></div>
                        <div className="flex justify-between text-sm"><span>Top Kapma</span><span className="font-bold">{player.stats.tackling}</span></div>
                    </div>
                </div>
                ...
                <div className="bg-slate-100 dark:bg-slate-900 p-4 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center">
                    <button onClick={() => { onClose(); /* Navigation to detail view can be added here if needed */ }} className="text-indigo-600 font-bold text-sm hover:underline">Tüm Özellikleri Gör</button>
